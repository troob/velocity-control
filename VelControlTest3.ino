...
void initNode(String id)
{
  ...
  Serial.println("Time1\tTime2\tInst\tSamp1\tPuls1\tSetV1\tActV1\tVErr1\tCmdV1\tCmdPMV1\tMtrCmd1\tSamp2\tPuls2\tSetV2\tActV2\tVErr2\tCmdV2\tCmdPMV2\tMtrCmd2");
}
...
void loop() 
{
  ...
  { 
    ...
    if(curTime - prevPulseTimes[i] > bounceTimes[i]) // if the noise is gone
    {
      validPulses[i] = true;
      ...
      if(newPulses[i])
      {
        ...
        prevPulseTimes[i] = curTime; // record the time of the first pulse in bounce group
...
//      Serial.println(" mm/s\n");
    ...
    { 
      for(int i=0; i < numEncoders; i++)
      {
        setAngVels[i] = inputString.substring(1, inputString.length()).toInt(); // get string after 'v'
...
void computeWheelVels()
{
  ...
  setAngVels[0] = (int) round( ( setFwdVel / minLinearRes + 0 ) * minAngularRes ); // [deg/s]
  setAngVels[1] = (int) round( ( setFwdVel / minLinearRes - 0 ) * minAngularRes ); // [deg/s]
...
//    locateTarget();
  ...
  if(setVelActive)
  {
    ...
    computeMtrCtrlSgnls(); // assume set vel active b/c vel ctrl test
  }
...
void observeSamples()
{
  //Serial.println("======Observe Samples======");
  for(int i=0; i < numEncoders; i++)
  {
  ...
  //angPos[i] = (int) round( minAngularRes * ( samples[i] % pulsesPerRev ) ); // [deg]
    ...
    angVels[i] = (int) round( minAngularRes * pulses[i] * pubVelRate); // [deg/s]
...
//  Serial.println(" deg/s)======");
  ...
  for(int i=0; i < numEncoders; i++)
  {
    velErrs[i] = cmdAngVels[i] - angVels[i];
    
    errs[i] = (long) round( velErrs[i] / degsPerRad * 256 ); // [rad/s]*256, generate error signal based on difference b/t measured rotVel and requested rotVel for the wheel. Note: multiply by 256 and later divide by 256 b/c operation in fixed point integer
  //  Serial.print("errs[");
  //  Serial.print(i);
  ...
  //  Serial.println(" rad/s");
    ...
    D[i] = (long) round( kd[i] * dAngVels[i] * pubVelRate ); // large when amount of change requested by PID controller is large, and small as signal approaches zero
  //  Serial.print("D[");
  //  Serial.print(i);
  //  Serial.print("] = kd[1][");
  //  Serial.print(i);
  ...
  //  Serial.print(" %/s + ");
    ...
    cmdPrcntMtrVltges[i] += (int) round( ( P[i] + D[i] ) / 256.0 ); // [%/s], + I[i]; // Increase or decrease speed of motor to force measured to equal requested rotVel
  //  Serial.print(P[i] / 256.0);
  //  Serial.print(" %/s + ");
  //  Serial.print(D[i] / 256.0);
  //  Serial.print(" %/s = ");
  ...
  //  Serial.println(" %/s");
    ...
    cmdPrcntMtrVltges[i] = clip(cmdPrcntMtrVltges[i], 100, -100); // [%/s]
  //  Serial.print("mtrOut[");
  //  Serial.print(i);
  //  Serial.print("] = ");
  //  Serial.print((int) round( mtrOut[i] / 256.0 ));
  //  Serial.println(" %/s");
    ...
    if(prevCmdPMV[i] > cmdPrcntMtrVltges[i])
      decelerating[i] = true;
    else
      decelerating[i] = false;

    prevCmdPMV[i] = cmdPrcntMtrVltges[i];
  }
}
